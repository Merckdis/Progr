...
public class Parser {
    ...
    //expr -> init | if_stmt | loopwhile | WR
    private boolean expr() {
        if (!(this.init() || this.if_stmt() || this.loopwhile() || this.WR()))
            return false;
        return true;
    }
    ...
    private boolean read(){
        if (!this.READ_KW())
            ...
            this.printExeption(this.tokens.get(this.pos).getValue(), "variable");
            return false;
    ...
    private boolean WRITE_KW(){
        if(this.tokens.get(this.pos).getTag().equals("WRITE")) {
    ...
    private boolean READ_KW(){
        if(this.tokens.get(this.pos).getTag().equals("READ")) {
            this.push(this.tokens.get(this.pos++));
            return true;
        }
        return false;
            ...
            if (token.getValue().equals("while") || token.getValue().equals("if")){
                ...
                if (token.getPriority() < this.buffer.get(this.buffer.size() - 1).getPriority()) {
                    if (token.getValue().equals(";"))
                        while (!(this.buffer.get(this.buffer.size() - 1).getValue().equals("=") ||
                                this.buffer.get(this.buffer.size() - 1).getValue().equals("write") ||
                                this.buffer.get(this.buffer.size() - 1).getValue().equals("read"))
                        )
